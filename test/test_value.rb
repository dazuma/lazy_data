...
describe LazyData::Value do
  describe "#get" do
    ...
    it "passes extra arguments" do
      cache = LazyData::Value.new do |arg1, arg2|
        arg1 + arg2
      end
    ...
    it "retries and succeeds" do
      ...
      cache = LazyData::Value.new retries: retries do
        count += 1
    ...
    it "waits for a delay before retrying" do
      count = 0
      retries = LazyData::Retries.new max_tries: 3, initial_delay: 0.1
      cache = LazyData::Value.new retries: retries do
        count += 1
    ...
    it "retries until a max time" do
      count = 0
      retries = LazyData::Retries.new max_time: 0.2, initial_delay: 0.13, max_tries: nil
      cache = LazyData::Value.new retries: retries do
        count += 1
        raise "whoops11"
      end
    ...
    it "includes time elapsed in a retry delay" do
      count = 0
      retries = LazyData::Retries.new max_tries: 3, initial_delay: 0.39, delay_includes_time_elapsed: true
      cache = LazyData::Value.new retries: retries do
        sleep 0.2
        count += 1
    ...
    it "does not allow thread re-entry" do
      ...
      assert_equal "deadlock: tried to call LazyData::Value#get from its own computation", err.message
    end
    ...
    it "returns an expiring value" do
      ...
      cache = LazyData::Value.new do
        count += 1
        LazyData.expiring_value 0.1, 10
      end
      assert_equal 10, cache.get
      assert_equal 1, count
      assert_equal 10, cache.get
    ...
    it "raises an expiring error" do
      ...
      cache = LazyData::Value.new do
        count += 1
        LazyData.raise_expiring_error 0.1, "count=#{count}"
      end
      err1 = assert_raises RuntimeError do
        cache.get
      end
  ...
  describe "#await" do
    ...
    it "repeatedly calls get until success" do
      count = 0
      retries = LazyData::Retries.new max_tries: 10
      cache = LazyData::Value.new retries: retries do
        count += 1
    ...
    it "uses the cache's retry interval" do
      count = 0
      retries = LazyData::Retries.new max_tries: 10, initial_delay: 0.2
      cache = LazyData::Value.new retries: retries do
        count += 1
        raise "whoops5"
      end
    ...
    it "honors transient_errors" do
      ...
      cache = LazyData::Value.new retries: retries do
        count += 1
        raise MyTestError if count < 3
        raise "whoops6"
      end
      err = assert_raises RuntimeError do
        cache.await transient_errors: [MyTestError], max_tries: 10
      end
  ...
  describe "#internal_state" do
    ...
    it "reflects error state with expiration" do
      cache = LazyData::Value.new do
        LazyData.raise_expiring_error 1, "whoops5"
      end
      err = assert_raises RuntimeError do
        cache.get
      end
